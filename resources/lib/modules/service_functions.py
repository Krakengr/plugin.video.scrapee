...
from apis.streamdb_api import sync_databases
...
class SyncDatabases:
	def run(self):
		logger(fen_str, 'Sync Service Starting')
		sync_databases()
		return logger(fen_str, 'Sync Service Finished')
...
class FirstRunActions:
	def run(self):
		...
		if kodi_version() < 20:
			ok_dialog(fen_str, 'Kodi 20 or above required[CR]Please update Kodi or uninstall Scrapee')
...
class PremiumExpiryCheck:
	def run(self):
		...
		try:
			self.run_check()
			if self.make_message(): ok_dialog('Scrapee - Premium Accounts Expiring...', '\n'.join(self.message))
			self.expired_premium = [i for i in self.days_remaining if i[1] == 0]
			if self.expired_premium and confirm_dialog(heading='Scrapee - Premium Accounts Expired', text='Disable Expired Accounts?', ok_label=32839, cancel_label=32840): self.revoke()
		except: pass
