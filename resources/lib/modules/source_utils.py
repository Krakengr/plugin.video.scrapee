...
from modules import scraper as cl
...
def post_link(url, post):
	return scraper.post(url, post=post)

def quality_estimate(width):
...
def build_link(url, provider = 'easynews', file_name = '', display_name = ''):
	video_quality = get_file_info(file_name)
	extraInfo = provider + ' (' + video_quality[0]
	if video_quality[1] != '':
		extraInfo += video_quality[1]
	extraInfo += ')'

	has_subs = True if provider == 'coverapi' or provider == 'learninguniverse' else False
	return {'name': file_name, 'display_name': display_name, 'quality': video_quality[0], 'size': 0, 'size_label': '%.2f GB' % 0,
			'extraInfo': extraInfo, 'url_dl': url, 'down_url': url, 'id': url, 'local': False, 'direct': True, 'source': 'easynews', 'scrape_provider': 'easynews', 'has_subs': has_subs, 'real_provider': provider}
