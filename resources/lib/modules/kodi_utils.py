...
addon_object = xbmcaddon.Addon('plugin.video.scrapee')
...
links_db = translatePath(path_join(database_path_raw, 'links.db'))
...
current_dbs = ('navigator.db', 'watched.db', 'favourites.db', 'traktcache4.db', 'maincache.db', 'metacache2.db', 'debridcache.db', 'providerscache2.db', 'settings.db', 'links.db')
...
def build_url(url_params):
	return 'plugin://plugin.video.scrapee/?%s' % urlencode(url_params)
...
def addon(addon_id='plugin.video.scrapee'):
...
def run_addon(addon='plugin.video.scrapee', block=False):
...
def disable_enable_addon(addon_name='plugin.video.scrapee'):
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.scrapee'):
	import time
	import sqlite3 as database
	try:
...
def toggle_language_invoker():
	close_all_dialog()
	sleep(100)
	addon_xml = translate_path('special://home/addons/plugin.video.scrapee/addon.xml')
...
def open_settings(query, addon='plugin.video.scrapee'):
	hide_busy_dialog()
	if query:
		try:
