...
class Sources():
	def __init__(self):
		...
		self.imdb_id = self.original_title = ''
	...
	def playback_prep(self, params=None):
		...
		self.folders_ignore_filters = get_setting('fen.results.folders_ignore_filters', 'false') == 'true'
		...
		self.original_title = self.meta.get('original_title')
		...
		self.hybrid_allowed, self.filter_size_method = self.filter_hdr in (0, 2), int(get_setting('fen.results.filter_size_method', '0'))
		self.include_unknown_size = get_setting('fen.results.include.unknown.size', 'false') == 'true'
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('fen.results.size.min_manual', '0'), '0') / 1000
			...
			if self.filter_size_method == 1:
				...
				max_size = ((0.125 * (0.90 * string_to_float(get_setting('fen.results.size.auto', '20'), '20'))) * duration)/1000
			elif self.filter_size_method == 2:
				max_size = string_to_float(get_setting('fen.results.size.manual', '10000'), '10000') / 1000
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('fen.%s.display_name' % i), i, source_folders_directory(self.media_type, i)) for i in folder_scrapers]
	...
	def _process_post_results(self):
		...
		res = self._process_scrapee_sources()
		
		if len(res['results']) > 0:
			if len(res['results']) == 1 and (self.background or self.autoplay):
				return self.play_file(res['results'])
			else:
				return self.play_source(res['results'])

		return self._no_results()
	
	def _process_scrapee_links(self, link):
		if link is not None:
			json_data['results'].append(link)

	def _process_scrapee_sources(self):
		show_busy_dialog()
		self.scrapers_dialog()
		#CoverApi
		#if self.coverapi_enabled:
		from scrapers import coverapi
		link = coverapi.source().coverapi(self.imdb_id, self.media_type, self.season, self.episode)
		self._process_scrapee_links(link)

		#Autoembed
		from scrapers import autoembed
		link = autoembed.source().autoembed(self.imdb_id, self.media_type, self.season, self.episode, self.tmdb_id)
		self._process_scrapee_links(link)

		#learninguniverse
		from scrapers import learninguniverse
		link = learninguniverse.source().learninguniverse(self.imdb_id, self.media_type, self.season, self.episode)
		self._process_scrapee_links(link)

		#torrents
		from scrapers import torrents
		link = torrents.source().torrent(self.imdb_id, self.original_title, self.media_type, self.season, self.episode, int(self.meta.get('year')))
		
		if link is not None and 'results' in link and len(link['results']) > 0:
			for l in link['results']:
				self._process_scrapee_links(l)
		#logger("json_data", str(json_data))
		return json_data
	...
	def get_search_year(self):
		...
		if custom_year: year = custom_year
			...
			if self.active_external and get_setting('fen.search.enable.yearcheck', 'false') == 'true':
	...
	def _special_filter(self, results, key, enable_setting):
		if key == self.hevc_filter_key and enable_setting in (0,2):
			hevc_max_quality = self._get_quality_rank(get_setting('fen.filter_hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def play_file(self, results, source={}):
		...
		try:
			...
			for count, item in enumerate(items, 1):
				...
				provider = item['scrape_provider']
				self.has_subs = resolve_item['has_subs']
				self.real_provider = resolve_item['real_provider']
				...
				resolve_item['has_subs'] = self.has_subs
				resolve_item['real_provider'] = self.real_provider
			...
			for count, item in enumerate(items, 1):
				try:
					...
					try:
						...
						if url:
							self.has_subs = item['has_subs'] if 'has_subs' in item else False
							self.real_provider = item['real_provider'] if 'real_provider' in item else ''
