...
def erase_bookmark(media_type, tmdb_id, season='', episode='', refresh='false'):
	try:
		...
		try:
			...
			streamdb_api.erase_bookmark(media_type, tmdb_id, season, episode)
		except:
			pass
		refresh_container(refresh == 'true')
	except: pass

def batch_erase_progress():
	tv_peogress_list = get_in_progress_episodes()
	mv_watched_list = get_watched_items('movie', 1)
	tv_watched_list = get_watched_items('tvshow', 1)
	mv_peogress_list = get_in_progress_movies('', 1)
	...
	try:
		...
		try:
			for i in tv_peogress_list:
				dbcur.execute("DELETE FROM progress where db_type = ? and media_id = ? and season = ? and episode = ?", ('episode', str(i['media_ids']['tmdb']), str(i['season']), str(i['episode'])) )
		except:
			pass

		try:
			for i in mv_peogress_list:
				dbcur.execute("DELETE FROM progress where db_type = ? and media_id = ? and season = ? and episode = ?", ('movie', str(i['media_id']), '', ''))
		except:
			pass
		
		try:
			for i in mv_watched_list:
				dbcur.execute("DELETE FROM watched_status where db_type = ? and media_id = ? and season = ? and episode = ?", ('movie', str(i['media_id']), '', ''))
		except:
			pass

		try:
			for i in tv_watched_list:
				dbcur.execute("DELETE FROM watched_status where db_type = ? and media_id = ? and season = ? and episode = ?", ('episode', str(i['media_ids']['tmdb']), '', ''))
		except:
			pass
...
def set_bookmark(params):
	try:
		...
		try:
			streamdb_api.set_bookmark(params, last_played)
		except:
			pass
...
def watched_status_mark(watched_indicators, media_type='', tmdb_id='', action='', season='', episode='', title='', from_sync = False):
	try:
		data_base = get_database(watched_indicators)
		last_played = get_last_played_value(data_base)
		dbcon = make_database_connection(data_base)
		...
		if action == 'mark_as_watched':
			...
			if from_sync is False:
				try:
					streamdb_api.mark_unwatched_status(media_type, tmdb_id, season, episode)
				except:
					pass
