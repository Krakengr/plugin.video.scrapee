...
database, delete_file, navigator_db, watched_db, favorites_db, links_db = kodi_utils.database, kodi_utils.delete_file, kodi_utils.navigator_db, kodi_utils.watched_db, kodi_utils.favorites_db, kodi_utils.links_db
...
def check_databases():
	...
	dbcon.close()
	# Links Cache
	dbcon = database.connect(links_db)
	dbcon.execute("""CREATE TABLE IF NOT EXISTS links (url text, type text, filename text, info text, imdb text, season text, episode text, source text, hash text, size text, added integer, expire integer, UNIQUE(imdb, source, hash, season, episode))""")
	dbcon.close()
